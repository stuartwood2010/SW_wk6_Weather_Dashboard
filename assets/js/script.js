...
function listCities() {
    savedCities = JSON.parse(localStorage.getItem('cityid'));
    ...
    if (savedCities === null) {} else {
        ...
        for (let i = 0; i < savedCities.length; i++) {
            let myCity = savedCities[i];
            myCityBtn = $('<button></button>');
            myCityBtn.attr('class', 'cityButtons');
            myCityBtn.attr('class', 'btn btn-primary col-10 mx-5 btn mt-1');
            myCityBtn.text(myCity);
            citiesList.append(myCityBtn);
            myCityBtn.click(function (event) {
                event.preventDefault();
                futureEl.empty();
                cityId = event.target.innerHTML;
                todayHeader.text(cityId);
                const requestUrl = 'https://api.openweathermap.org/data/2.5/weather?q=' + cityId + '&units=imperial&appid=a31971324859b8bfdaba0df04f7db013';
                fetch(requestUrl)
                    .then(function (response) {
